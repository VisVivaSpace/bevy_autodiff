...
fn test_autodiff_quadratic() {
    ...
    assert!((ad.eval(f) - 9.0).abs() < 1e-10);
}
...
fn test_autodiff_quadratic_derivative() {
    ...
    // f'(2) = 6
    let f = quadratic(&mut ad, x);
    assert!((ad.derivative(f, x, 1) - 6.0).abs() < 1e-10);
}
...
fn test_autodiff_two_vars() {
    ...
    assert!((ad.eval(f) - 11.0).abs() < 1e-10);
}
...
fn test_autodiff_transcendental() {
    ...
    assert!((ad.eval(f) - 1.0).abs() < 1e-10);
}
...
fn test_autodiff_transcendental_derivative() {
    ...
    // f'(0) = 1 - 0 = 1
    let f = transcendental(&mut ad, x);
    assert!((ad.derivative(f, x, 1) - 1.0).abs() < 1e-10);
}
...
fn test_autodiff_division() {
    ...
    assert!((ad.eval(f) - 3.0).abs() < 1e-10);
}
...
fn test_autodiff_reciprocal() {
    ...
    assert!((ad.eval(f) - 0.25).abs() < 1e-10);
}
...
fn test_autodiff_reciprocal_derivative() {
    ...
    let f = reciprocal(&mut ad, x);
    assert!((ad.derivative(f, x, 1) - (-0.25)).abs() < 1e-10);
}
...
fn test_autodiff_exp_derivative() {
    ...
    let f = exponential(&mut ad, x);
    assert!((ad.derivative(f, x, 1) - std::f64::consts::E).abs() < 1e-10);
}

#[test]
fn test_autodiff_ln() {
    let mut ad = AutoDiff::new();
    let x = ad.var(std::f64::consts::E);
...
fn test_autodiff_ln_derivative() {
    ...
    let f = logarithm(&mut ad, x);
    assert!((ad.derivative(f, x, 1) - 0.5).abs() < 1e-10);
}
...
fn test_autodiff_exp_ln_identity() {
    ...
    assert!((ad.eval(f) - 5.0).abs() < 1e-10);
}
...
fn test_autodiff_pow() {
    ...
    assert!((ad.eval(f) - 8.0).abs() < 1e-10);
}
...
fn test_autodiff_pow_square() {
    ...
    assert!((ad.eval(f) - 25.0).abs() < 1e-10);
}
...
fn test_autodiff_sin_of_cos() {
    ...
    assert!((ad.eval(f) - 1.0_f64.sin()).abs() < 1e-10);
}
...
fn test_autodiff_sin_of_cos_derivative() {
    ...
    let f = sin_of_cos(&mut ad, x);
    assert!((ad.derivative(f, x, 1) - 0.0).abs() < 1e-10);
}
...
fn test_autodiff_triple_nested() {
    ...
    assert!((ad.eval(f) - expected).abs() < 1e-10);
}
...
fn test_autodiff_cubic_derivatives() {
    ...
    assert!((ad.eval(f) - 8.0).abs() < 1e-10);
    assert!((ad.derivative(f, x, 1) - 12.0).abs() < 1e-10);
    assert!((ad.derivative(f, x, 2) - 12.0).abs() < 1e-10);
    assert!((ad.derivative(f, x, 3) - 6.0).abs() < 1e-10);
}
...
fn test_autodiff_quartic_derivatives() {
    ...
    assert!((ad.eval(f) - 1.0).abs() < 1e-10);
    assert!((ad.derivative(f, x, 1) - 4.0).abs() < 1e-10);
    assert!((ad.derivative(f, x, 2) - 12.0).abs() < 1e-10);
    assert!((ad.derivative(f, x, 3) - 24.0).abs() < 1e-10);
    assert!((ad.derivative(f, x, 4) - 24.0).abs() < 1e-10);
}
...
fn test_autodiff_exp_higher_derivatives() {
    ...
    assert!((ad.derivative(f, x, 1) - 1.0).abs() < 1e-10);
    assert!((ad.derivative(f, x, 2) - 1.0).abs() < 1e-10);
    assert!((ad.derivative(f, x, 3) - 1.0).abs() < 1e-10);
    assert!((ad.derivative(f, x, 4) - 1.0).abs() < 1e-10);
}
...
fn test_autodiff_integer_literals() {
    ...
    assert!((ad.eval(f) - 17.0).abs() < 1e-10);
}
...
fn test_autodiff_integer_literals_derivative() {
    ...
    // f'(x) = 6x + 2, f'(2) = 14
    let f = with_integers(&mut ad, x);
    assert!((ad.derivative(f, x, 1) - 14.0).abs() < 1e-10);
}
...
fn test_autodiff_constant_only() {
    ...
    assert!((ad.eval(f) - 42.0).abs() < 1e-10);
}
...
fn test_autodiff_deep_nesting() {
    ...
    assert!((ad.eval(f) - (-0.28)).abs() < 1e-10);
}
...
fn test_autodiff_tan_at_pi_4() {
    let mut ad = AutoDiff::new();
    let x = ad.var(std::f64::consts::FRAC_PI_4);
...
fn test_autodiff_tan_identity() {
    ...
    assert!((ad.eval(f1) - ad.eval(f2)).abs() < 1e-10);
}
...
fn test_autodiff_asin_at_half() {
    ...
    // asin(0.5) = π/6
    let f = arc_sin(&mut ad, x);
    assert!((ad.eval(f) - std::f64::consts::FRAC_PI_6).abs() < 1e-10);
}
...
fn test_autodiff_acos() {
    ...
    assert!((ad.eval(f) - std::f64::consts::FRAC_PI_2).abs() < 1e-10);
}
...
fn test_autodiff_asin_acos_sum() {
    ...
    assert!((ad.eval(f1) + ad.eval(f2) - std::f64::consts::FRAC_PI_2).abs() < 1e-10);
}
...
fn test_autodiff_atan_at_one() {
    ...
    // atan(1) = π/4
    let f = arc_tan(&mut ad, x);
    assert!((ad.eval(f) - std::f64::consts::FRAC_PI_4).abs() < 1e-10);
}
...
fn test_autodiff_acosh_at_two() {
    ...
    // acosh(2) = ln(2 + sqrt(3))
    let f = arc_cosh(&mut ad, x);
    assert!((ad.eval(f) - 2.0_f64.acosh()).abs() < 1e-10);
}
...
fn test_autodiff_acosh_derivative() {
    ...
    let f = arc_cosh(&mut ad, x);
    assert!((ad.derivative(f, x, 1) - (1.0 / 3.0_f64.sqrt())).abs() < 1e-10);
}
...
fn test_autodiff_atanh_at_half() {
    ...
    let f = arc_tanh(&mut ad, x);
    assert!((ad.eval(f) - 0.5_f64.atanh()).abs() < 1e-10);
}
...
fn test_autodiff_sin_of_atan() {
    ...
    assert!((ad.eval(f) - std::f64::consts::FRAC_1_SQRT_2).abs() < 1e-10);
}
